...
window.onload=function(){
    ...
    function visualizza(){
        ...
        if(color)
        {
            alert("Ha vinto " + color);
            disabilitaDivs();
        }
    }

    function cambiaColore(){
        if(getComputedStyle(_divTurno).backgroundColor == YELLOW)
    ...
    function controllaVincita(){
        ...
        for (let i = 0; i < RIGHE; i++) {
            for (let j = 0; j < COLONNE - 3; j++) {
                ...
                //se la cella corrente è stata modificata dinamicamente (cioè non è grigia)
                if(_currentDiv.style.backgroundColor != "")
                {
                    ...
                    if(_currentDiv.style.backgroundColor == _next1.style.backgroundColor &&
                        _currentDiv.style.backgroundColor == _next2.style.backgroundColor &&
                        _currentDiv.style.backgroundColor == _next3.style.backgroundColor)
                    {
